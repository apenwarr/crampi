...
def test_gitdb():
    ...
    l2 = { 1:u1, 2:u1b, 3:u2 }
    msg = 'test commit'
    c1 = g.commit_set('one', t1, l1, msg)
    c1b = g.commit_set('one', t1, l1, msg)
    c2 = g.commit_set('two', t1, l2, msg)
    print c1,c1b,c2

    WVPASSNE(c1, c1b)
    WVPASSNE(c1, c2)

    WVPASSEQ(('one', t1, l1, msg, None), g.commit(c1))
    WVPASSEQ(('one', t1, l1, msg, None), g.commit(c1b))
    WVPASSEQ(('two', t1, l2, msg, None), g.commit(c2))
    ...
    m1 = g.commit_set('two', t1, l2, msg, c1b)
    m2 = g.commit_set('two', t1, l2, msg, c1)
    m3 = g.commit_set('two', t1, l2, msg, c2)
    m4 = g.commit_set('one', t1, l2, msg, c2)
    m5 = g.commit_set('one', t1, l2, msg, c1)
