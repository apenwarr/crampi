...
def test_save_load_entries():
    ...
    try:
        e1.save_commit(gdb, 'tsle', 'message')
    except:
        WVPASS("expected exception")
    else:
        WVFAIL("expected exception")
    e1.assign_missing_uuids(gdb)
    c1 = e1.save_commit(gdb, 'tsle', 'message2')
    WVPASS(c1)
    WVPASSEQ(gdb.commitid_latest('tsle'), c1)
    c1b = e1.save_commit(gdb, 'tsle', 'message3')
    WVPASS(c1b)
    WVPASSNE(c1, c1b)
    WVPASSEQ(gdb.commitid_latest('tsle'), c1b)
    t1 = e1.save_tree(gdb)

    (r1a, t1a, lids1a, msg1a, m1a) = gdb.commit(c1)
    (r1b, t1b, lids1b, msg1b, m1b) = gdb.commit(c1b)
    WVPASSEQ(t1, t1a)
    WVPASSEQ(t1, t1b)
    WVPASSEQ(r1a, 'tsle')
    WVPASSEQ(r1b, 'tsle')
    WVPASSEQ(msg1a, 'message2')
    WVPASSEQ(msg1b, 'message3')
